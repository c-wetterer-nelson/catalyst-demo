...
void Attributes::fillPressureIC()
{
  ...
  {
    unsigned int* pts = this->GridPtr->GetCellPoints(c);
    double x = this->GridPtr->GetPoint(pts[0])[0];
    double y = this->GridPtr->GetPoint(pts[0])[1];

    if((x-20)*(x-20) + (y-45)*(y-45) < 36)
    {
      this->Pressure[c] = 2.f;
    }
    if((x-49)*(x-49) + (y-45)*(y-45) < 36)
    {
      this->Pressure[c] = 2.f;
    }

    double dst = (x-34.5)*(x-34.5) + (y-32)*(y-32);
    if(dst > 225 && dst < 400 && y < 32)
    {
      this->Pressure[c] = 2.f;
    }
  }
